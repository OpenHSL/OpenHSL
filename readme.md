...
---------------------
## Формирование ГСИ
Первоначально требуется иметь JSON файл, хранящий метаинформацию для формирования ГСИ. Такой файл генерируется один раз для определенного гиперспектрометра и далее может использовать для формирования ГСИ, полученных этим гиперспектрометром. 

При его отсутствии потребуется:
1) Сформировать калибровочный набор данных, используя перестраиваемый лазер;
2) Воспользоваться GUI интерфейсом модуля калибровки, выполнив команду `python openhsl/gui/device/hs_device_gui.py`
3) Следуя инструкции в Wiki проекта сгенерировать JSON файл.

При наличии такого файла (как в представленных ниже наборах данных) можно приступать непосредственно к процессу формирования ГСИ.
...
Для обучения и инференса нейросетевых классификаторов (поддерживаются реализации на PyTorch и Tensorflow 2.x) можно воспользоваться:
...
---------------------
## Демонстрационный файл метаобучения

В [файле](https://github.com/OpenHSL/OpenHSL/blob/main/MetaLearning%20demo.ipynb) приведена демонстрация на закрытом наборе данных по применению метаобучения для уточнения нейросетевого анализа.
...
Веса модели доступны по [ссылке](https://huggingface.co/OpenHSL/soil_ssftt)
#### Формирование ГСИ из набора кадров
Для формирования данного ГСИ из набора кадров при помощи CLI требуется:
1) изменить в файле *cli_demo_builder.py* значения нижепредставленных параметров следующим образом:
...
3) Сформировать итоговое ГСИ из двух вышеописанных:
   - 3.1 изменить в файле *uav_dataset_builder.py* значения нижепредставленных параметров следующим образом:
      - PATH_TO_UAV_1 = '.../uav_1.npy', где вместо ... указать директорию содержащую сформированное первое ГСИ и сопутствующую метаинформацию
